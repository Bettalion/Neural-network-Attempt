...
def populate(sample= 10):
  ...
  try:
    ...
    if os.path.exists('DATA.txt') == False:
      ...
      while naming ==True:
        print('enter')
        if os.path.exists(f'DATA ({count}).txt') == False:
          log = open(f'DATA ({count}).txt','w')
          naming=False
        else:
          count+=1
  except:
...
def CreateScript(Cases,InputNodes,grades):
  ...
  ############
  # testing
  trs=CA(InputNodes,0,TL,'TRs',False,True)
  tss=CA(InputNodes,1,TL,'TSs',False,True)
  gens=CA(InputNodes,2,TL,'GENs',False,True)
  atens=CA(InputNodes,3,TL,'ATENs',False,True)
  weas=CA(InputNodes,4,TL,'WEAs',False,True)
  famss=CA(InputNodes,5,TL,'FAMSs',False,True)
  eths=CA(InputNodes,6,TL,'ETHs',False,True)
  hquals=CA(InputNodes,7,TL,'HQUALs',False,True)
